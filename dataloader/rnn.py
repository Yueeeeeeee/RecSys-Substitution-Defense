...
class RNNDataloader():
    ...
    def _get_eval_dataset(self, mode):
        if mode == 'val':
            ...
            dataset = RNNTestDataset(self.train, self.val, self.test, self.max_len, 
                        self.test_negative_samples)
        return dataset


class RNNTrainDataset(data_utils.Dataset):
    def __init__(self, u2seq, max_len, max_history=10000):
        # self.u2seq = u2seq
        ...
        for u in sorted(u2seq.keys()):
            seq = u2seq[u][-max_history:]
            for i in range(1, len(seq)):
                self.all_seqs += [seq[:-i][-self.max_len:]]
